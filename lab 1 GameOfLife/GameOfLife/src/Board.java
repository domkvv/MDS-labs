    ...
    private void initialize(int length, int height) {
        ...
        for (int x = 0; x < points.length; ++x) {
            for (int y = 0; y < points[x].length; ++y) {
                ...
                points[x][y].setPointMode(boardMode);
            }
        }
        ...
        for (int x = 0; x < points.length; ++x) {
            for (int y = 0; y < points[x].length; ++y) {
                ...
                if (boardMode == 1 || boardMode == 2 || boardMode == 3) {
                    for (int i = -1; i < 2; i++) {
                        for (int j = -1; j < 2; j++) {
                            if ((i != 0 || j != 0) && x + i >= 0 && x + i < points.length && y + j >= 0 && y + j < points[x].length) {
                ...
                for (y = 0; y < points[x].length; ++y) {
                    if (boardMode == 1 || boardMode == 2 || boardMode == 3) {
                        ...
                        if (points[x][y].getState() != 0) {
                            switch (points[x][y].getState()) {
                                case 1:
                                    g.setColor(new Color(0xA3E4F3));
                                    break;
                                case 2:
                                    g.setColor(new Color(0x74C1E7));
                                    break;
                                case 3:
                                    g.setColor(new Color(0x64A3F3));
                                    break;
                                case 4:
                                    g.setColor(new Color(0x3469D7));
                                    break;
                                case 5:
                                    g.setColor(new Color(0x113792));
                                    break;
                                case 6:
                                    g.setColor(new Color(0xffffff));
                                    break;
